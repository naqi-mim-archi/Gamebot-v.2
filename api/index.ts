...
app.post('/api/discord/share', async (req, res) => {
  const { webhookUrl, gameName, prompt, gameId } = req.body;
  
  if (!webhookUrl || !gameName || !gameId) {
    return res.status(400).json({ error: "Missing required parameters" });
  }

  try {
    // Format a beautiful rich embed for the Discord message
    const embed = {
      title: "🎮 New Game Published: " + gameName,
      description: prompt,
      // URL makes the title clickable, taking them straight to the game!
      url: `https://gamebot.studio/play/${gameId}`, 
      color: 0x10B981, // Emerald Green matching GameBot's branding
      fields: [
        {
          name: "Play Now",
          value: `[Click here to play the game!](https://gamebot.studio/play/${gameId})`
        }
      ],
